...
export default function FollowersPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // stable order: by display_name||username, case-insensitive
      ...
      // 4) my following set for buttons
      if (me) {
        const { data: edges } = await supabase
          .from('follows')
          .select('following_id')
          .eq('follower_id', me.id);
        setMyFollowing(new Set((edges ?? []).map((r: any) => String(r.following_id))));
      } else {
  ...
  async function toggleFollow(targetId: string, currentlyFollowing: boolean) {
    ...
    if (currentlyFollowing) {
      ...
      await supabase
        .from('follows')
        .insert({ follower_id: me.id, following_id: targetId });
